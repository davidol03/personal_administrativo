...
@section('content')
<div class="container">
    <h1 class="mb-4 animated-title text-center">Citas</h1>
    <div class="text-center mb-4">
        <a href="{{ route('citas.create') }}" class="btn btn-primary add-btn">
    ...
    <div class="custom-table-container">
        ...
        @foreach($citas as $cita)
            <div class="table-row animated-row">
                <div style="flex: 1;">{{ $cita->id_cita }}</div>
                <div>{{ $cita->fecha_hora_cita }}</div>
                <div>{{ $cita->medico->nombre_medico ?? 'No asignado' }} {{ $cita->medico->apellido_medico ?? '' }}</div>
                <div>{{ $cita->estatus->nombre_estatus ?? 'Desconocido' }}</div>
                <div>{{ $cita->estatusUsuario->nombre_usuario ?? 'Desconocido' }}</div>
                <div class="actions">
                    <a href="{{ route('citas.edit', $cita->id_cita) }}" class="btn btn-sm btn-warning edit-btn">
