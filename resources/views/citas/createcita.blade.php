...
@section('content')
<div class="container">
    <h1 class="text-center animated-title">Crear Cita</h1>

    <form action="{{ route('citas.store') }}" method="POST" class="mt-4">
        @csrf
        <div class="form-group">
            <label for="id_cita">ID Cita</label>
            <input type="number" name="id_cita" id="id_cita" class="form-control custom-input" required>
        </div>

        <div class="form-group mt-3">
            <label for="fecha_hora_cita">Fecha y Hora de la Cita</label>
            <input type="datetime-local" name="fecha_hora_cita" id="fecha_hora_cita" class="form-control custom-input" required>
        </div>
        ...
        <div class="text-center mt-4 mb-5">
            <button type="submit" class="btn btn-primary submit-btn">Crear Cita</button>
        </div>
    </form>
</div>
