...
@section('content')
<div class="container">
    <h1 class="text-center animated-title">Editar Cita</h1>

    <form action="{{ route('citas.update', $cita->id_cita) }}" method="POST" class="mt-4">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="fecha_hora_cita">Fecha y Hora de la Cita</label>
            <input type="datetime-local" class="form-control custom-input" name="fecha_hora_cita" id="fecha_hora_cita" value="{{ $cita->fecha_hora_cita }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control custom-select" name="id_medico" id="id_medico" required>
                @foreach($medicos as $medico)
                    <option value="{{ $medico->id_medico }}" {{ $cita->id_medico == $medico->id_medico ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <select class="form-control custom-select" name="id_estatus" id="id_estatus" required>
                @foreach($estatus as $estatusItem)
                    <option value="{{ $estatusItem->id_estatus }}" {{ $cita->id_estatus == $estatusItem->id_estatus ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <select class="form-control custom-select" name="id_estatus_usuario" id="id_estatus_usuario" required>
                @foreach($estatusUsuarios as $estatusUsuario)
                    <option value="{{ $estatusUsuario->id_estatus_usuario }}" {{ $cita->id_estatus_usuario == $estatusUsuario->id_estatus_usuario ? 'selected' : '' }}>
        ...
        <div class="text-center mt-4">
            <button type="submit" class="btn btn-primary submit-btn">Actualizar Cita</button>
        </div>
    </form>
</div>
