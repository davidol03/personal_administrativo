...
@section('content')
<div class="container">
    <h1 class="mb-4 animated-title text-center">Especialidades</h1>

    <div class="text-center mb-4">
        <a href="{{ route('especialidades.create') }}" class="btn btn-primary add-btn">
    ...
    <div class="custom-table-container">
        ...
        @foreach($especialidades as $especialidad)
            <div class="table-row animated-row">
                <div style="flex: 1;">{{ $especialidad->id_especialidad }}</div>
                <div>{{ $especialidad->nombre_especialidad }}</div>
                <div>{{ $especialidad->descripcion }}</div>
                <div>{{ $especialidad->estatus->nombre_estatus ?? 'Desconocido' }}</div>
                <div>{{ $especialidad->estatusUsuario->nombre_usuario ?? 'Desconocido' }}</div>
                <div class="actions">
                    <a href="{{ route('especialidades.edit', $especialidad->id_especialidad) }}" class="btn btn-sm btn-warning edit-btn">
                        <img src="{{ asset('images/edit.gif') }}" alt="Edit gif" class="action-gif"> Editar
                    </a>

                    <form action="{{ route('especialidades.destroy', $especialidad->id_especialidad) }}" method="POST" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
