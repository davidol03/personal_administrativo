...
@section('content')
<div class="container">
    <h1 class="mb-4 animated-title text-center">Estatus</h1>
    <div class="text-center mb-4">
        <a href="{{ route('estatus.create') }}" class="btn btn-primary add-btn">
            <i class="bi bi-plus-circle"></i> Agregar Estatus
        </a>
    </div>
    ...
    <div class="custom-table-container">
        ...
        @foreach($estatus as $status)
            <div class="table-row animated-row">
                <div>{{ $status->id_estatus }}</div>
                <div>{{ $status->nombre_estatus }}</div>
                <div class="actions">
                    <a href="{{ route('estatus.edit', $status->id_estatus) }}" class="btn btn-sm btn-warning edit-btn">
                        <img src="{{ asset('images/edit.gif') }}" alt="Edit gif" class="action-gif"> Editar
                    </a>
                    <form action="{{ route('estatus.destroy', $status->id_estatus) }}" method="POST" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
