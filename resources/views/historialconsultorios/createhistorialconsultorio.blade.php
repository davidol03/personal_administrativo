...
@section('content')
<div class="container">
    <h1 class="text-center animated-title">Crear Historial de Consultorio</h1>
    
    <form action="{{ route('historialconsultorios.store') }}" method="POST" class="mt-4">
        @csrf
        
        <div class="form-group">
            <label for="id_historial">ID Historial</label>
            <input type="number" name="id_historial" id="id_historial" class="form-control custom-input" required>
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <select name="id_medico" id="id_medico" class="form-control custom-select" required>
                @foreach($medicos as $medico)
                    <option value="{{ $medico->id_medico }}">{{ $medico->nombre_medico }} {{ $medico->apellido_medico }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <input type="datetime-local" name="fecha_uso" id="fecha_uso" class="form-control custom-input" required>
        </div>
        ...
        <div class="text-center mt-4">
            <button type="submit" class="btn btn-primary submit-btn">Crear Historial</button>
        </div>
    </form>
</div>
